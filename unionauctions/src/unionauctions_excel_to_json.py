...
def excel_to_json(filepath):
    ...
    df = pd.read_excel(filepath, engine='openpyxl')  # or engine='xlrd' for older .xls files
    ...
    for _, row in df.iterrows():
        ...
        raw_id = row.get("MainQuestionId")
        if raw_id is None or str(raw_id).strip() == "":
            question_id = None  # or use a default like -1 if needed
        else:
    ...
    #final_json = {
    #    "overallSummary": None,  # You can populate this later once all sections are answered
    #    "overallVerdict": None,  # You can populate this later once all sections are answered
...
if __name__ == "__main__":
    inputpath = r"input/UnionAuctionQA_ENG_17Sep2025.xlsx"
    output = r"output/output.json"
